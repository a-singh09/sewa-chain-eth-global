...
async function main() {
  ...
  const URIDRegistry = await ethers.getContractFactory('URIDRegistry');
  ...
  const DistributionTracker = await ethers.getContractFactory('DistributionTracker');
  ...
  console.log('✅ DistributionTracker deployed to:', distributionTrackerAddress);
  ...
  try {
    ...
    console.log('📊 DistributionTracker - Total distributions:', totalDistributions.toString());
    
    // Test cooldown periods
    const foodCooldown = await distributionTracker.DEFAULT_FOOD_COOLDOWN();
    console.log('⏰ Food cooldown period:', (Number(foodCooldown) / 3600).toString(), 'hours');
  ...
  console.log(`NEXT_PUBLIC_DISTRIBUTION_TRACKER_ADDRESS=${distributionTrackerAddress}`);
  console.log('─'.repeat(50));
  ...
  console.log(`Estimated Cost: ${ethers.formatEther(totalGasUsed * 20000000000n)} ETH (at 20 gwei)`);
  console.log('─'.repeat(50));
