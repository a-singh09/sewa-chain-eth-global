...
import { ethers, JsonRpcProvider, Contract, Wallet, TransactionReceipt, formatEther } from 'ethers';
import type { 
  IContractService, 
  TransactionResult, 
  Family, 
...
import { getContractConfig, CONTRACT_LIMITS, COOLDOWN_PERIODS } from '@/config/contracts';
...
export class ContractService implements IContractService {
  ...
  async registerFamily(uridHash: string, familySize: number): Promise<TransactionResult> {
    try {
      // Validate inputs
      ...
      if (familySize < CONTRACT_LIMITS.MIN_FAMILY_SIZE || familySize > CONTRACT_LIMITS.MAX_FAMILY_SIZE) {
        throw new Error(`Family size must be between ${CONTRACT_LIMITS.MIN_FAMILY_SIZE} and ${CONTRACT_LIMITS.MAX_FAMILY_SIZE}`);
      }
      ...
      // Estimate gas
      const gasEstimate = await this.uridRegistry.registerFamily.estimateGas(uridHash, familySize);
  ...
  async recordDistribution(params: DistributionParams): Promise<TransactionResult> {
    try {
      // Validate inputs
      if (!params.uridHash || params.uridHash === '0x' || params.uridHash.length !== 66) {
        throw new Error('Invalid URID hash format');
      }
      
      if (!params.volunteerNullifier || params.volunteerNullifier === '0x' || params.volunteerNullifier.length !== 66) {
        throw new Error('Invalid volunteer nullifier format');
      }

      if (params.quantity < CONTRACT_LIMITS.MIN_QUANTITY || params.quantity > CONTRACT_LIMITS.MAX_QUANTITY) {
        throw new Error(`Quantity must be between ${CONTRACT_LIMITS.MIN_QUANTITY} and ${CONTRACT_LIMITS.MAX_QUANTITY}`);
      }
      ...
      // Check eligibility
      const eligibility = await this.checkEligibility(params.uridHash, params.aidType);
      if (!eligibility.eligible) {
        const hoursRemaining = Math.ceil(eligibility.timeUntilEligible / 3600);
        throw new Error(`Family not eligible for ${params.aidType}. Please wait ${hoursRemaining} hours.`);
      }
      ...
      // Estimate gas
      const gasEstimate = await this.distributionTracker.recordDistribution.estimateGas(
        params.uridHash,
  ...
  async checkEligibility(uridHash: string, aidType: AidType): Promise<EligibilityResult> {
    try {
      if (!uridHash || uridHash === '0x' || uridHash.length !== 66) {
        throw new Error('Invalid URID hash format');
      }
      ...
      const result = await this.distributionTracker.checkEligibility(uridHash, aidTypeValue);
  ...
  async getDistributionHistory(uridHash: string): Promise<ContractDistribution[]> {
    try {
      if (!uridHash || uridHash === '0x' || uridHash.length !== 66) {
        return [];
      }

      const distributions = await this.distributionTracker.getDistributionHistory(uridHash);
  ...
  async getVolunteerStats(nullifier: string): Promise<VolunteerStats> {
    try {
      ...
      const distributionCount = await this.distributionTracker.getVolunteerDistributionCount(nullifier);
...
export const getContractService = (network: 'testnet' | 'mainnet' = 'testnet'): ContractService => {
