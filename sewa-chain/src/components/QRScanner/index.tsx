...
export function QRScanner({
  ...
  const [permissionStatus, setPermissionStatus] =
    useState<CameraPermissionStatus>("checking");
  ...
  const renderPermissionDenied = () => (
    <div className="flex flex-col items-center justify-center p-8 bg-red-50 rounded-lg border border-red-200">
      ...
      <p className="text-red-700 text-center mb-4">
        Please enable camera permissions in your browser settings to scan QR
        codes
      </p>
      <Button
        onClick={requestCameraPermission}
  ...
  const renderScanningInterface = () => (
    <div className="relative">
      ...
      <div className="relative rounded-lg overflow-hidden bg-black">
        <div id="qr-reader" ref={qrReaderRef} className="w-full h-80 md:h-96" />
  ...
  return (
    <div className={`qr-scanner ${className}`}>
      ...
      {/* Manual URID Entry Fallback */}
      <ManualURIDEntry onSubmit={handleSuccessfulScan} />
    </div>
  );
}

// Manual URID Entry Component
function ManualURIDEntry({ onSubmit }: { onSubmit: (urid: string) => void }) {
  const [manualURID, setManualURID] = useState("");
  const [isExpanded, setIsExpanded] = useState(false);

  const handleManualSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const trimmedURID = manualURID.trim().toUpperCase();

    if (URIDService.validateURID(trimmedURID)) {
      onSubmit(trimmedURID);
      setManualURID("");
      setIsExpanded(false);
    }
  };

  const isValidURID = URIDService.validateURID(manualURID.trim().toUpperCase());
      ...
      {!isExpanded ? (
        ...
        <div className="space-y-4">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-600">
              Enter the 16-character URID from the family's document
            </p>
          </div>
          ...
          <form onSubmit={handleManualSubmit} className="space-y-4">
            <div>
              ...
              <input
                type="text"
                value={manualURID}
                onChange={(e) => setManualURID(e.target.value.toUpperCase())}
                placeholder="e.g., A1B2C3D4E5F6G7H8"
                maxLength={16}
                className={`
                  w-full px-3 py-3 border rounded-lg text-center font-mono text-lg tracking-wider
              ...
              <div className="mt-2 text-xs text-gray-500 text-center">
                Format: 16 uppercase letters and numbers (A-F, 0-9)
              </div>
                ...
                <div className="mt-2 text-center">
                  {isValidURID ? (
                    <div className="flex items-center justify-center text-green-600">
                      ...
                      <span className="text-sm">Valid URID format</span>
                    </div>
                  ) : (
                    <div className="flex items-center justify-center text-red-600">
                      <ExclamationTriangleIcon className="w-4 h-4 mr-1" />
                      <span className="text-sm">
                        {manualURID.length !== 16
                          ? `${manualURID.length}/16 characters`
                          : "Invalid format - use A-F and 0-9 only"}
                      </span>
                    </div>
                  )}
                </div>
