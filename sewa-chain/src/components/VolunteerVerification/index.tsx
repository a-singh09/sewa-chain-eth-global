...
const errorMessages: Record<VerificationErrorCode, string> = {
  [VerificationErrorCode.MINIKIT_UNAVAILABLE]: 'World App is required for volunteer verification',
  ...
  [VerificationErrorCode.ALREADY_REGISTERED]: 'This identity is already registered as a volunteer',
  [VerificationErrorCode.NETWORK_ERROR]: 'Network error occurred. Please try again.',
...
export function VolunteerVerification({ 
  ...
  const handleVolunteerVerification = async () => {
    try {
      ...
      const errorCode = error.message as VerificationErrorCode || VerificationErrorCode.VERIFICATION_FAILED;
    ...
    <div className={`w-full max-w-md mx-auto space-y-4 ${className}`}>
      ...
      {verificationState !== 'idle' && (
        <LiveFeedback
          ...
          message={
            verificationState === 'success' ? 'Successfully verified as volunteer!' :
            verificationState === 'failed' ? (errorDetails?.message || 'Verification failed') :
