...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.hashedAadhaar || !body.location || !body.familySize || !body.contactInfo) {
      return NextResponse.json({
        status: 'error',
        message: 'Missing required fields: hashedAadhaar, location, familySize, contactInfo'
      } as GenerateURIDResponse, { status: 400 });
    }
    ...
    return NextResponse.json({
      ...
      contractTxHash
    } as GenerateURIDResponse, { status: 200 });
    ...
    return NextResponse.json({
      status: 'error',
      message: error instanceof Error ? error.message : 'Internal server error during URID generation'
    } as GenerateURIDResponse, { status: 500 });
  }
}
 ...
 */
async function registerFamilyOnBlockchain(uridHash: string, familyData: FamilyData): Promise<string> {
